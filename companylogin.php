    ...
    <body>
        <form action="" method="post">
            <div class="box">
                <h2>Company Login</h2>
                <div class="login">
                    <input type="text" name="username" placeholder="company_name">
                    <label>Username</label>
                </div>
                <div class="login">
                    <input type="password" name="password" placeholder="company_name">
        ...
        <?php
				...
				if(isset($_POST['log']))
				{
                    ...
                    else
                    {
			    	    ...
			    	    $query="select * from company_form where companyname='$name'";
			    	    $result=mysqli_query($con,$query);
				
			         while($row=mysqli_fetch_array($result))
				        {
                            if(($name==$row[0]) && ($pwd==$row[0]))
                            {
